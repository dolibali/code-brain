...
8. **与 GBrain 对比，Code Brain 做了正确的减法（去掉 Postgres/embedding/enrichment），但在某些地方减过头了**——特别是页面版本控制、stale 检测和 link graph 遍历，这些在代码知识场景下同样有价值。
...
**问题所在：** [§7.5 自动 upsert 长期知识的规则](file:///Users/zhangrich/work/code-brain/docs/DESIGN.md#L876-L892)
...
**问题所在：** [§6.6 性能目标](file:///Users/zhangrich/work/code-brain/docs/DESIGN.md#L802-L811)
...
- 用户体感是"这个 Agent 比不接 Code Brain 时慢了很多"

对比：GBrain 的搜索目标虽然没有明确写延迟，但其 FTS5 + pgvector hybrid search 设计的本地路径通常在亚秒级。

**影响范围：** 直接影响产品可用性和用户留存。如果 Agent 因为等搜索而显著变慢，用户会关掉 Code Brain。
...
**问题所在：** [§6.1 目标](file:///Users/zhangrich/work/code-brain/docs/DESIGN.md#L730-L740) + [§5.2 pages_fts](file:///Users/zhangrich/work/code-brain/docs/DESIGN.md#L676-L687)
...
**问题所在：** [§5.3 Write-through 同步](file:///Users/zhangrich/work/code-brain/docs/DESIGN.md#L697-L713)
...
**问题所在：** [§7.6 去重与更新策略](file:///Users/zhangrich/work/code-brain/docs/DESIGN.md#L894-L907)
...
**问题所在：** [§9.5 Gemini CLI](file:///Users/zhangrich/work/code-brain/docs/DESIGN.md#L1144-L1155)
...
1. **Gemini CLI 如何知道要调用 code-brain？** Claude Code 有 `CLAUDE.md`，Cursor 有 `.cursor/rules/`，但 Gemini CLI 的工具注册和规则注入方式是什么？文档没有给出可操作的配置示例
2. **CLI 调用时的上下文传递**：Gemini CLI 运行时，`context_path` 如何自动传入？是需要 Gemini CLI 的 prompt 里明确编排 `code-brain search --context-path $(pwd)` 吗？
...
**问题所在：** [§4.3 scope_refs 结构](file:///Users/zhangrich/work/code-brain/docs/DESIGN.md#L368-L392)
...
**问题所在：** [§8.3 配置文件](file:///Users/zhangrich/work/code-brain/docs/DESIGN.md#L974-L1077)
...
3. 增加 `code-brain init` 交互式配置向导，类似 GBrain 的 init wizard
4. 提供 `code-brain config validate` 命令
...
GBrain 有 `page_versions` 表和 `gbrain history/diff/revert` 命令。Code Brain 的 Markdown-as-truth 虽然可以依赖 git 版本控制，但文档没有显式说明这一点。如果 brain repo 不在 git 管理下（虽然不推荐），页面修改就不可回溯。
...
Code Brain 记录了 `created_at` 和 `updated_at`，但没有主动检测过时知识的机制。一条 6 个月前的 `architecture` 页面可能已经严重不符合当前代码实际结构。
...
| **数据迁移策略** | 如果用户已有 Markdown 知识文件但没有 Code Brain 的 frontmatter 格式 | 增加 `import` / `migrate` 工具或说明 |
...
这个决策是正确的。与 GBrain 的 Postgres-first 相比，Code Brain 的受众是本地开发者，`git + Markdown + SQLite` 三件套完全契合目标用户画像——无需额外数据库服务，知识文件可读可改可审计，索引层出问题可以从 Markdown 全量重建。

这也是 Code Brain 与其他"记忆 MCP"产品的核心差异化所在。
...
与 GBrain 的 `people / companies / deals / meetings / projects / ideas / concepts / writing / programs / ...` 16+ 类型相比，Code Brain 做了正确的领域聚焦。
...
这是 Code Brain 相对于静态知识库的核心增量点。GBrain 有 `timeline_entries`，但 Code Brain 把开发过程提升为独立页面类型，使得"发生了什么变更"和"现在的结论是什么"形成对等的两层知识，这是适合代码场景的设计。
