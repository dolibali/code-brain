...
5. **Agent 提供判断，Code Brain 提供持久化、检索与索引**
...
- `reindex` 是恢复与重建路径
- `diff / commit / Agent summary` 只是 Agent 整理记忆时可参考的原始材料，不是 Code Brain 核心服务接口
...
- Code Brain 不从代码状态自动推断“任务已完成”
- Code Brain 不在后台自行触发整理流程
...
但它们都不应成为 Code Brain v1 核心服务职责，也不应进入当前正式工具面。
...
```bash
codebrain serve
codebrain search "electron sandbox crash" --project kilo-code
codebrain get issue/electron-sandbox-crash --project kilo-code
codebrain list --project kilo-code --types issue,practice
codebrain put change/2026/2026-04-18-preload-bridge --project kilo-code --file ./change.md
codebrain put practice/preload-bridge-rule --project kilo-code --file ./practice.md
codebrain link --project kilo-code --from change/2026/2026-04-18-preload-bridge --to issue/electron-sandbox-crash --relation updates
codebrain reindex --project kilo-code
codebrain project register --id kilo-code --root ~/work/kilo-code --remote github.com/your-org/kilo-code --main-branch main
...
- 若当前环境以 CLI 为主，则通过 `codebrain` 命令访问相同 operations
...
- 默认通过 `codebrain` CLI 访问相同 operations
- 若后续运行环境支持 MCP 或等价工具桥接，可复用同一 `codebrain serve`
...
codebrain put change/2026/2026-04-18-preload-bridge --context-path "$(pwd)" --file ./change.md
codebrain link --project kilo-code --from change/2026/2026-04-18-preload-bridge --to practice/preload-bridge-rule --relation implements
```
